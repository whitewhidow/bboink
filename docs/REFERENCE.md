...
The deep reference: the on-device UI, cracking/sync details, and firmware internals.
New here? Start with the **[README](../README.md)** and the
**[Getting-started guide](GETTING-STARTED.md)** — this doc is for when you want the details.
...
Captures reach the cracking services through a tiny **relay** you host once, so the board (or
...
reflashed over USB. A tag also refreshes the [web flasher](https://whitewhidow.github.io/bboink/flasher/)'s
version list (the `publish-flasher` job commits each release's bins under `flasher/fw/`, keeping the latest two).
