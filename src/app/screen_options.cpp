...
static void buildRow(int f) {
    ...
    if (d.isText) {
        ...
        if (f == OPT_KEY || f == OPT_OHC || f == OPT_PWN)   // keys are sensitive -> hide value
...
// Download progress line (throttled) drawn mid-screen during the OTA fetch.
static void updProgress(size_t done, size_t total) {
    static uint32_t last = 0;
    if (millis() - last < 200) return;
    last = millis();
    char l[32];
    if (total) snprintf(l, sizeof(l), "%u%%  %uKB", (unsigned)(done * 100 / total), (unsigned)(done / 1024));
    else       snprintf(l, sizeof(l), "%u KB", (unsigned)(done / 1024));
    M5.Display.fillRect(0, PORK_DISPLAY_H / 2 - 4, PORK_DISPLAY_W, 22, TFT_BLACK);
    M5.Display.setTextSize(2); M5.Display.setTextDatum(top_center); M5.Display.setTextColor(TFT_CYAN, TFT_BLACK);
    M5.Display.drawString(l, PORK_DISPLAY_W / 2, PORK_DISPLAY_H / 2 - 4);
}
...
static void sdUpdateFlow() {
    ...
    if (r.ok) {
        ...
        App::footer("rebooting to launcher...");
        delay(1800);
        ESP.restart();
    } else {
        App::centerMsg(r.error, TFT_RED);
